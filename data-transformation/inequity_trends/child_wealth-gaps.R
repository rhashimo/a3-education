...
child_status %>% 
  group_by(WINDEX) %>% 
  mutate(windex = WINDEX,
         category = C003,
         share = n / sum(n)) %>% 
  ungroup() %>% 
  select(-c(n, C003, WINDEX)) %>% 
  select(category, windex, share) %>%
  write.csv("data/output/child_status.csv", row.names = FALSE)

child_status %>% 
  spread(key = C003, value = n) %>% 
  write.csv("data/output/child_status_1.csv", row.names = FALSE)
...
child_inst <- child_2018 %>% 
  ...
  mutate(C006 = recode(C006, "1" = "Government", "2" = "Private", "3" = "Madrasah"," 4" = "Other"),
         C006 = fct_relevel(C006, "Government", "Private", "Madrasah", "Other")) %>% 
...
child_inst %>% 
  ...
  mutate(windex = WINDEX,
         ...
         share = n / sum(n)) %>% 
  ungroup() %>% 
  select(-c(n, C006, WINDEX)) %>% 
  select(category, windex, share) %>%
  write.csv("data/output/child_inst.csv", row.names = FALSE)

qggplot(child_inst, aes(x = WINDEX, y = n, fill = C006)) +
  geom_bar(stat = "identity", position = "fill") +
  scale_y_continuous(labels = percent) +
  theme_minimal() +
  scale_fill_brewer(palette = "Spectral") +
